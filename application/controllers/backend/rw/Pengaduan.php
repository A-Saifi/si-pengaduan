...
class Pengaduan extends Backend {
	...
	{
		$rw = $this->session->userdata('admin');

		$this->load->model('Riwayat_alamat_model');
		$alamat = $this->Riwayat_alamat_model->get_alamat($rw['id_pdk']);

		$this->load->model('Disposisi_pengaduan_model');
		$pengaduan = $this->Disposisi_pengaduan_model->get_disposisi_valid($rw['id_rj']);
		...
		$data = [
			...
			'small' => 'Wilayah RW-'.ucwords(strtolower($alamat['nama_rw'])).', '.ucwords(strtolower($alamat['nama_kel'])).', '.ucwords(strtolower($alamat['nama_kec'])),
			'crumb' => [
												['label'=>'Ketua RW','link'=>'admin/rw'],
												['label'=>'Pengaduan','link'=>'admin/rw/pengaduan'],
												['label'=>'Valid','link'=>'admin/rw/pengaduan/valid'],
											],

		];

		$this->layout->load_backend('backend/rw/pengaduan/valid/daftar', $data);
	}
	...
	public function detail($id_pengaduan)
	{
		...
		$data = [
			...
			'crumb' => [
												['label'=>'Ketua RW','link'=>'admin/rw'],
												['label'=>'Pengaduan','link'=>'admin/rw/pengaduan'],
												['label'=>'Detail Pengaduan','link'=>'admin/rw/pengaduan/detail/'.$id_pengaduan]
		...
		$this->layout->load_backend('backend/rw/pengaduan/detail', $data);
	}
	...
	public function disposisi()
	{
      ...
      'crumb' =>  [
                    ['label'=>'Ketua RW','link'=>'admin/rw'],
										['label'=>'Pengaduan','link'=>'admin/rw/pengaduan'],
                    ['label'=>'Disposisi','link'=>'admin/rw/disposisi'],
                      ],
      'small' => 'Daftar Pengaduan Yang Telah Didisposisikan',
			'disposisi' => $pengaduan,
    ];

		$this->layout->load_backend('backend/rw/pengaduan/disposisi/daftar', $data);
	}
	...
	public function daftar()
	{
      ...
      'small' => 'Daftar Pengaduan Pada Wilayah RW-'.ucwords(strtolower($alamat['nama_rw'])).', '.ucwords(strtolower($alamat['nama_kel'])).', '.ucwords(strtolower($alamat['nama_kec'])),
			'pengaduan' => $pengaduan,
    ];

		$this->layout->load_backend('backend/rw/pengaduan/ok/daftar', $data);
	}
